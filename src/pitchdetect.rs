...
const NUM_FRAMES_CONCAT:usize = 16;
...
impl PitchEstimatorThread{
    pub fn new(sr:usize, snapshot_ref:BusReader<[(f32, f32); SNAPSHOT_BUFFLEN]>, f0_tx:Bus<(f32, f32, bool, f32)>, spec_tx:Bus<[f32;NUM_FREQS]>, cthresh:f32) -> PitchEstimatorThread{
        ...
        loop{
            ...
            for i in 0..NUM_FRAMES_CONCAT{
                for j in 0..SNAPSHOT_BUFFLEN{
                    multi_frame_snapshot[i*SNAPSHOT_BUFFLEN + j] = self.waveform_snapshot_buffer[i as isize][j];
                }
            }


            let timestamp = multi_frame_snapshot[0].0;
            let amps = multi_frame_snapshot.map(|el| el.1);
